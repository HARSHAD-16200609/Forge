    ...
    <DropdownMenu>
      <DropdownMenuTrigger className='bg-secondary flex items-center gap-2 rounded-lg px-3 py-2.5'>
        ...
        <div className='flex flex-col gap-1 text-start leading-none'>
          <span className='max-w-[17ch] truncate text-sm leading-none font-semibold'>{data?.workspace.workspaceName}</span>
          <span className='text-muted-foreground max-w-[20ch] truncate text-xs'>{data?.workspace.visibility}</span>
        </div>
      </DropdownMenuTrigger>
      <DropdownMenuContent className='w-66'>
        <DropdownMenuLabel>My Workspaces</DropdownMenuLabel>
        {workspaces && workspaces.map(workspace => (
          <DropdownMenuItem key={workspace.workspace.id} onClick={() => {
            ...
            <div className='flex items-center gap-2'>
              ...
              <div className='flex flex-col gap-1 text-start leading-none'>
                <span className='max-w-[17ch] truncate text-sm leading-none font-semibold'>{workspace.workspace.workspaceName}</span>
                <span className='text-muted-foreground max-w-[20ch] truncate text-xs'>{ }</span>
              </div>
            </div>
