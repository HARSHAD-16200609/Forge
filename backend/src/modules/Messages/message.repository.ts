...
import {
  ...
  toWsMessageDTO,
  WsMessageDTO,
} from "../../websockets/types/wsMessageDTO";
...
class MessageRepository {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const attachments = await tx.upload.updateMany({
        data: {
          ...
          conversationId:
            "conversationId" in messageObj ? messageObj.conversationId : null,
        },
        where: {
          id: {
            in: uploadIds,
      ...
      if (attachments.count !== uploadIds.length)
        throw new BadRequestError(
          "Some uipload Id's are invlaid or already attached",
        );
  ...
  async getMessages(
    channelId: string,
    pagination: { cursor?: string | undefined; limit: number },
  ): Promise<WsMessageDTO[]> {
