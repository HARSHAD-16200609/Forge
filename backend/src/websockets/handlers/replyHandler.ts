...
async function reply(ws: WebSocket, message: WebSocketMessage): Promise<void> {
  ...
  const { workspaceId, parentMsgId, entityId, content, uploadIds, entityType } =
    messagePayload.data;
  ...
  if (parent.deletedAt) {
    sendWs(
      ...
      WsResponse.fail(
        ...
        StatusCodes.BAD_REQUEST,
        "BAD_REQUEST",
        "Parent message is already deleted",
      ),
    );
    return;
  }
  ...
  if (entityType === "channel") {
    ...
    const conversation = await conversationRepository.conversationExists(
      entityId,
      userMetadata.userId,
    );
    if (!conversation) {
  ...
  const createdReply = await messageRepository.createReply(
    messageObj,
    parentMsgId,
    uploadIds,
    userMetadata.userId,
  ...
  await notificationService.createMentions(
    ...
    entityType === "channel"
      ? {
          ...
          workspaceId,
          channelId: entityId,
          snippet: mentionSnippet(content),
        }
      : {
          kind: "conversation",
          workspaceId,
