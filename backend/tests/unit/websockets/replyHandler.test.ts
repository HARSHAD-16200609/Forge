  ...
  async (importOriginal) => {
    const actual =
      await importOriginal<
        typeof import("../../../src/modules/Notifications/notifications.service")
      >();
    return {
      ...actual,
      notificationService: {
...
vi.mock(
  "../../../src/websockets/utility/wsResponse",
  async (importOriginal) => {
    const actual =
      await importOriginal<
        typeof import("../../../src/websockets/utility/wsResponse")
      >();
    return {
      ...actual,
...
const lastFrame = () =>
  sentFrames[sentFrames.length - 1]!.response as {
    type: string;
    success: boolean;
    statusCode: number;
    data?: unknown;
...
describe("replyHandler.reply", () => {
  ...
  it("creates a channel reply and broadcasts to channel subscribers excluding the sender", async () => {
    authMocks();
    vi.mocked(channelRepository.memberExists).mockResolvedValue({
      id: "cm-1",
    } as never);
    vi.mocked(messageRepository.messageExists).mockResolvedValue(
      channelParent as never,
    );
    vi.mocked(messageRepository.createReply).mockResolvedValue(
      createdReply as never,
    );
  ...
  it("creates mentions for a channel reply with the channel snippet and reply id", async () => {
    authMocks();
    vi.mocked(channelRepository.memberExists).mockResolvedValue({
      id: "cm-1",
    } as never);
    ...
    const mentionMemberIds = [
      "2f3a8c1e-5b4d-4e6f-9a1c-2d3e4f5a6b7c",
      "6d7e8f9a-1b2c-3d4e-af6a-7b8c9d0e1f2a",
    ];
    ...
    await replyHandler.reply(ws, {
      ...
      payload: {
        ...
        content: JSON.stringify([
          { type: "paragraph", data: { text: "hello @team" } },
        ]),
        mentions: mentionMemberIds,
      },
    });

    expect(notificationService.createMentions).toHaveBeenCalledTimes(1);
    expect(notificationService.createMentions).toHaveBeenCalledWith(
      createdReply.id,
      metadata.userId,
      mentionMemberIds,
      {
        ...
        workspaceId: WORKSPACE_ID,
        channelId: CHANNEL_ID,
        snippet: expect.stringContaining("hello @team"),
      },
    );
  ...
  it("creates mentions for a conversation reply with the conversation snippet", async () => {
    authMocks();
    vi.mocked(conversationRepository.conversationExists).mockResolvedValue({
      id: CONVO_ID,
    } as never);
    ...
    const mentionMemberIds = ["2f3a8c1e-5b4d-4e6f-9a1c-2d3e4f5a6b7c"];
    ...
    await replyHandler.reply(ws, {
      ...
      payload: {
        ...
        entityType: "conversation",
        content: JSON.stringify([
          { type: "paragraph", data: { text: "bye @team" } },
        ]),
        mentions: mentionMemberIds,
      },
    });
    ...
    expect(notificationService.createMentions).toHaveBeenCalledWith(
      ...
      mentionMemberIds,
      {
        ...
        workspaceId: WORKSPACE_ID,
        conversationId: CONVO_ID,
        snippet: expect.stringContaining("bye @team"),
      },
    );
...
describe("eventRouter.dispatch reply path", () => {
  ...
  it("dispatches channel.message.reply to the registered handler without throwing", async () => {
    ...
    const response = otherFrames[0]!.response as {
      type: string;
      success: boolean;
      statusCode: number;
    };
  ...
  it("dispatches conversation.message.reply to the registered handler", async () => {
    ...
    const otherFrames = framesFor(otherWs);
    expect(otherFrames).toHaveLength(1);
    expect(otherFrames[0]!.response as { type: string }).toMatchObject({
      type: WsEvent.ConversationMessageReply,
    });
